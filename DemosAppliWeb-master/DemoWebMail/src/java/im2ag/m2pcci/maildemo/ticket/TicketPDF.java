...
public class TicketPDF {
    ...
    public static byte[] createPDF_AsByteArray(String titulaire, Epreuve epreuve, int nbPlaces,
            String logoFileName) throws IOException, COSVisitorException, WriterException {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            try ( // Définition d'un objet contentStream destiné à "contenir" le document qui va être créée.
                ...
                PDXObjectImage logoImage = new PDPixelMap(document, logo);
                ...
                contentStream.moveTextPositionByAmount(140, 670); // positionnement du curseur là où le texte doit être affiché
                contentStream.drawString("Ticket N°" + String.format("%06d", noTicket)); // affichage du texte
                contentStream.endText();

                // affichage du nom du titulaire du billet
                contentStream.beginText();
                contentStream.moveTextPositionByAmount(140, 650);
                contentStream.drawString(titulaire);
                contentStream.endText();

                // affichage du nom de l'épreuve
                contentStream.beginText();
                contentStream.moveTextPositionByAmount(140, 630);
                contentStream.drawString("Epreuve: " + epreuve.getNom());
                contentStream.endText();

                // affichage de la date de l'épreuve
                contentStream.beginText();
                contentStream.moveTextPositionByAmount(140, 610);
                contentStream.drawString("Date: " + epreuve.getFormatedDate());
                contentStream.endText();

                // affichage du nombre de places achetées
                contentStream.beginText();
                contentStream.moveTextPositionByAmount(140, 590);
                contentStream.drawString("Nombre de places: " + nbPlaces);
                contentStream.endText();

                // affichage du prix total
                double prixTotal = epreuve.getPrixUnitaire() * nbPlaces;
                contentStream.beginText();
                contentStream.moveTextPositionByAmount(140, 570);
                contentStream.drawString("Prix total: " + String.format("%5.2f", prixTotal) + " Euros");
                contentStream.endText();
...
//                // écriture de l'image dans le fichier pdf
                PDXObjectImage ximage = new PDPixelMap(document, createQRC(data,100));
