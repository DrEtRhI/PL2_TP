...
public class UtilisateursDAO {
     ...
     * d'un utilisateur, et si l'authentification est correcte retourne un
     * objet Utilisateur correspondant à l'utilisateur authentifié.
     * Si l'identifiant n'est pas connu dans la table COMPTES_UTILISATEUR cette
     * méthode retourne la valeur null.
     * Si l'identifiant est connu mais le mot de passe est incorrect une exception
     * de type MotPasseIncorrectException est levée.
     *
     ...
     * la table COMPTES_UTILISATEURS et si le mot de passe est correct. 
     * Renvoie null si l'identifiant  (l'utilsiateur) n'existe pas dans 
     * la table COMPTES_UTILISATEUR.
     *
                ...
                try (ResultSet rs = pstmt.executeQuery()){
                    if (rs.next()){
                        if(rs.getString("PASSWORD").equals(motPasse)){
                            user = new Utilisateur(rs.getString(1),rs.getString(2), rs.getString(3), rs.getString(4));
                        }else{
